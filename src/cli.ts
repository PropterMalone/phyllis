...
import {
	getHoursUntilWeeklyReset,
	readRateLimits,
	run,
	runBurn,
} from "./runner.ts";
import { shouldAlertBurnPoint } from "./scheduler.ts";
...
Usage:
  ...
  phyllis queue add --name <n> --size <S|M|L|XL> --prompt <p> --dir <d> [--priority <n>] [--preflight <cmd>] [--burn-point-only]
  phyllis queue done <name-or-id>
  phyllis queue angel --dir <d> [--priority <n>] [--queue <path>]
  phyllis run [--queue <path>] [--dry-run]
  phyllis fire-burn [--queue <path>] [--dry-run]
  phyllis burn-status [--queue <path>]
...
Commands:
  ...
  fire-burn   Manually fire held burn-point tasks
  burn-status Show burn-point status and any held reserve task
	...
	for (let i = startIdx; i < args.length; i++) {
		switch (args[i]) {
			...
			case "--preflight":
				...
				break;
			case "--burn-point-only":
				taskBurnPointOnly = true;
				break;
			case "--hours":
				hours = Number(args[++i]);
				break;
...
async function runQueue(parsed: ResolvedArgs): Promise<void> {
	...
	if (subcommand === "add") {
		...
		const task = await addTask(queuePath, {
			...
			burn_point_only: taskBurnPointOnly,
		});
		console.log(`added task: ${task.name} (${task.id})`);
		if (task.burn_point_only) {
			console.log("  held (burn-point-only) — fire with: phyllis fire-burn");
		}
		return;
	}
...
function printRunnerResult(result: Awaited<ReturnType<typeof run>>): void {
...
async function runScheduler(parsed: ResolvedArgs): Promise<void> {
	...
	printRunnerResult(result);
}

async function runFireBurn(parsed: ResolvedArgs): Promise<void> {
	const result = await runBurn({
		queuePath: parsed.resolvedQueuePath,
		logPath: parsed.resolvedLogPath,
		dryRun: parsed.dryRun,
		config: parsed.config,
...
async function runBurnStatus(parsed: ResolvedArgs): Promise<void> {
	const p = paths(parsed.config);
	const [rateLimits, hoursUntilReset, heldTask] = await Promise.all([
		readRateLimits(p.windowState, p.rateLimits),
		getHoursUntilWeeklyReset(p.windowState, p.rateLimits),
		nextBurnPointTask(parsed.resolvedQueuePath),
	]);

	const weeklyPct = rateLimits?.sevenDayPct ?? null;
	const alert = shouldAlertBurnPoint({
		weeklyPct,
		hoursUntilReset,
	...
	console.log("\n  Burn-point status\n");
	if (weeklyPct == null) {
		console.log("  weekly: unknown (no live rate-limit data)");
	} else {
		console.log(`  weekly: ${weeklyPct}% used, ${100 - weeklyPct}% unused`);
	}
	console.log(
		`  5h window: ${rateLimits ? `${rateLimits.fiveHourPct}% used` : "unknown"}`,
	);
	if (hoursUntilReset == null) {
		console.log("  reset: unknown");
	} else {
		console.log(
			`  reset: in ${Math.round(hoursUntilReset)}h (~${Math.floor(hoursUntilReset / 5)} windows left)`,
		);
	}
	console.log(`  past burn point (alertable): ${alert ? "yes" : "no"}`);
	console.log(
		`  held reserve: ${heldTask ? `${heldTask.name} (${heldTask.id})` : "none"}`,
	);
}
...
async function main(): Promise<void> {
	...
	switch (resolved.command) {
		...
		case "run":
			...
			break;
		case "fire-burn":
			await runFireBurn(resolved);
			break;
		case "burn-status":
			await runBurnStatus(resolved);
			break;
		case "throttle":
		...
		case "digest":
			...
			break;
		case "proxy": {
			// Rename /proc/comm so earlyoom can shield the daemon by name. MUST match
			// the --avoid regex in /etc/default/earlyoom (host config, untracked) and
			// stay <=15 chars (the /proc/comm limit) or earlyoom stops matching it.
			process.title = "phyllis-proxy";
