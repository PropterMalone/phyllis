...
describe("extractProject", () => {
	it("extracts project name from session ID", () => {
		expect(extractProject("-home-user-Projects-my-app")).toBe("my-app");
	});

	it("handles nested project paths", () => {
		expect(extractProject("-home-user-Projects-acme-widgets")).toBe(
			"acme-widgets",
		);
	});

	it("returns raw ID if no Projects match", () => {
		expect(extractProject("-home-user")).toBe("-home-user");
	});
});
...
describe("buildProjectSummary", () => {
	const sessions: CcusageSession[] = [
		{
			sessionId: "-home-user-Projects-my-app",
			totalTokens: 100000,
			totalCost: 50,
		...
		{
			sessionId: "-home-user-Projects-starcounter",
			totalTokens: 50000,
			totalCost: 25,
	...
	it("sorts by cost descending", () => {
		const summaries = buildProjectSummary(sessions);
		expect(summaries[0].project).toBe("my-app");
...
describe("renderProjectTable", () => {
	it("includes header, data rows, and total", () => {
		const summaries = buildProjectSummary([
			{
				sessionId: "-home-user-Projects-foo",
				totalTokens: 1000,
				totalCost: 10,
				lastActivity: "2026-03-30",
				modelsUsed: [],
