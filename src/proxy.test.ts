...
describe("proxy integration", () => {
	...
	it("emits a proxy-snapshot on the first request when headers present", async () => {
		await setupMock((_req, res) => {
			res.writeHead(200, {
				"content-type": "application/json",
				...RATE_LIMIT_HEADERS,
		...
		const entries = logContent
			...
			.split("\n")
			.map((l) => JSON.parse(l) as CalibrationEntry);
		const snap = entries.find((e) => e.source === "proxy-snapshot");
		expect(snap).toBeDefined();
		expect(snap?.notes).toContain("5h=");
		expect(snap?.notes).toContain("7d=");
		expect(snap?.rate_limits?.five_hour_pct).toBe(42);
		expect(snap?.rate_limits?.seven_day_pct).toBe(15);
	});

	it("suppresses snapshots within 5 minutes when utilization barely moves", async () => {
		// Two back-to-back requests with same utilization — only the first should snapshot.
		await setupMock((_req, res) => {
			res.writeHead(200, {
				"content-type": "application/json",
				...RATE_LIMIT_HEADERS,
		...
		const snaps = logContent
			.trim()
			.split("\n")
			.filter((l) => l.length > 0)
			.map((l) => JSON.parse(l) as CalibrationEntry)
			.filter((e) => e.source === "proxy-snapshot");
		expect(snaps).toHaveLength(1);
	});

	it("emits a fresh snapshot when 5h utilization jumps by >= 1pp", async () => {
		// First mock: utilization 0.42
		await setupMock((_req, res) => {
			res.writeHead(200, {
				"content-type": "application/json",
				...RATE_LIMIT_HEADERS,
		...
		// Swap upstream to one with 0.44 (2pp jump) — should snapshot.
		await new Promise<void>((r) => mockUpstream.close(() => r()));
		await new Promise<void>((r) => proxyServer.close(() => r()));
		mockUpstream = createServer((_req, res) => {
			res.writeHead(200, {
		...
		expect(snaps[snaps.length - 1].rate_limits?.five_hour_pct).toBe(44);
	});

	it("emits a fresh snapshot when 7d utilization jumps by >= 1pp", async () => {
		await setupMock((_req, res) => {
			res.writeHead(200, {
				"content-type": "application/json",
				...RATE_LIMIT_HEADERS,
		...
		const snaps = logContent
			...
			.filter((e) => e.source === "proxy-snapshot");
		expect(snaps.length).toBeGreaterThanOrEqual(2);
		expect(snaps[snaps.length - 1].rate_limits?.seven_day_pct).toBe(17);
	});

	it("reset events keep proxy-reset source — not downgraded to snapshot", async () => {
		// First request: high utilization
		await setupMock((_req, res) => {
			res.writeHead(200, {
				"content-type": "application/json",
		...
		// Second request: reset (drop >= 0.10)
		await new Promise<void>((r) => mockUpstream.close(() => r()));
		await new Promise<void>((r) => proxyServer.close(() => r()));
		mockUpstream = createServer((_req, res) => {
			res.writeHead(200, {
		...
		const entries = logContent
			...
			.split("\n")
			.map((l) => JSON.parse(l) as CalibrationEntry);
		expect(entries.some((e) => e.source === "proxy-reset")).toBe(true);
	});

	it("429 events keep proxy-429 source — not downgraded to snapshot", async () => {
		await setupMock((_req, res) => {
			res.writeHead(429, {
				"content-type": "application/json",
				"retry-after": "30",
		...
		const entries = logContent
			...
			.split("\n")
			.map((l) => JSON.parse(l) as CalibrationEntry);
		expect(entries.some((e) => e.source === "proxy-429")).toBe(true);
		// On a 429, the path is a transition emission — the snapshot path is bypassed.
		expect(entries.some((e) => e.source === "proxy-snapshot")).toBe(false);
	});
