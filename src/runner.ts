...
/**
 * Compute per-task cost metrics from before/after window snapshots.
 * Exported for testability. Uses block-token deltas, falling back to
 * undefined fields when ccusage data isn't available.
 */
export function computeCostMetrics(
	...
	const result: ReturnType<typeof computeCostMetrics> = {};
	if (before.blockTokens != null && after.blockTokens != null) {
		const delta = after.blockTokens - before.blockTokens;
		// Negative means a new block started during the task — count only the
		// post-rollover portion (after.blockTokens), since the pre-rollover
		// portion belongs to the previous window.
		result.tokens_used = delta >= 0 ? delta : after.blockTokens;
	}
	if (before.blockCost != null && after.blockCost != null) {
		const delta = after.blockCost - before.blockCost;
		result.cost_used = delta >= 0 ? delta : after.blockCost;
	}
	if (before.fiveHourPct != null && after.fiveHourPct != null) {
		const delta = after.fiveHourPct - before.fiveHourPct;
		// Negative = window reset; record 0 (we can't attribute pre-reset usage)
		result.five_hour_pct_delta = delta >= 0 ? delta : 0;
	}
	if (before.sevenDayPct != null && after.sevenDayPct != null) {
		const delta = after.sevenDayPct - before.sevenDayPct;
		result.seven_day_pct_delta = delta >= 0 ? delta : 0;
	}
	return result;
}
...
// Base backoff after a failure — let rate limits clear before retrying.
// Exponential with jitter so synchronized cron retries don't dogpile.
const FAILURE_BACKOFF_BASE_MS = 60 * 1000;
const FAILURE_BACKOFF_MAX_MS = 10 * 60 * 1000; // 10min ceiling

/**
 * Exponential backoff with full jitter.
 * - failureCount 1 -> ~60s (0-120s with jitter)
 * - failureCount 2 -> ~120s (0-240s)
 * - failureCount 3 -> ~240s (0-480s, capped at MAX)
 * Full jitter (random between 0 and 2*base*2^(n-1)) prevents synchronized
 * retry storms when multiple cron jobs fail at the same instant.
 * Pure function — random source is injectable for testability.
 */
export function computeBackoffMs(
	...
	const exponential = FAILURE_BACKOFF_BASE_MS * 2 ** (failureCount - 1);
	const capped = Math.min(exponential * 2, FAILURE_BACKOFF_MAX_MS * 2);
...
export async function getHoursUntilWeeklyReset(
	windowStatePath: string,
	rateLimitsPath: string,
): Promise<number | null> {
	// Try proxy state first — but only if fresh, matching readRateLimits's
	// staleness contract. A stale proxy reset epoch combined with fresh
	// fallback percentages would feed the burn-point math mixed-freshness data.
	try {
		const content = await readFile(windowStatePath, "utf-8");
		const state = JSON.parse(content) as {
			capturedAt?: string;
		...
		if (state.capturedAt && state.sevenDayResetAt) {
			const age = Date.now() - new Date(state.capturedAt).getTime();
			if (age < PROXY_STALENESS_MS) {
...
// Epoch-ms of the weekly reset, used to detect new weekly cycles for
// alert deduplication. Distinct from getHoursUntilWeeklyReset (a duration).
// Gated on proxy freshness and fallback-aware (rate-limits.json) so the dedup
// key stays stable across proxy<->fallback source flips — a flipping key would
// look like a new weekly cycle and trigger spurious re-alerts.
async function readSevenDayResetAt(
	windowStatePath: string,
	rateLimitsPath: string,
): Promise<number | null> {
	try {
		...
		if (state.capturedAt && state.sevenDayResetAt) {
			const age = Date.now() - new Date(state.capturedAt).getTime();
			if (age < PROXY_STALENESS_MS) {
				const ms = new Date(state.sevenDayResetAt).getTime();
	...
	try {
		const { mtimeMs } = await stat(rateLimitsPath);
		if (Date.now() - mtimeMs <= PROXY_STALENESS_MS) {
			const content = await readFile(rateLimitsPath, "utf-8");
			const data = JSON.parse(content) as {
				seven_day?: { resets_at?: number };
			};
			if (data.seven_day?.resets_at) return data.seven_day.resets_at * 1000;
		}
	} catch {
		// fall through
	}
...
interface BurnAlertState {
	sevenDayResetAt: number | null;
	atMs: number;
}

async function readBurnAlertState(
	burnAlertPath: string,
): Promise<BurnAlertState | null> {
	try {
		const content = await readFile(burnAlertPath, "utf-8");
		const state = JSON.parse(content) as BurnAlertState;
		if (typeof state.atMs === "number") return state;
	} catch {
		// no prior alert
	}
	return null;
...
async function writeBurnAlertState(
	burnAlertPath: string,
	state: BurnAlertState,
): Promise<void> {
	try {
		await mkdir(join(burnAlertPath, ".."), { recursive: true });
		await writeFile(burnAlertPath, JSON.stringify(state, null, "\t"));
	} catch {
		// best-effort
	}
}

// Independent of the scheduling decision: detect past-burn-point + a held
// reserve task, and send a (rate-limited) Signal alert. Never fires the task,
// never throws.
async function maybeAlertBurnPoint(
	queuePath: string,
	...
	try {
		const heldTask = await nextBurnPointTask(queuePath);
		const shouldAlert = shouldAlertBurnPoint({
			weeklyPct: rateLimits?.sevenDayPct ?? null,
			hoursUntilReset: hoursUntilWeeklyReset,
			hasHeldTask: !!heldTask,
		});
		if (!shouldAlert || dryRun || !heldTask || !rateLimits) return;

		const sevenDayResetAt = await readSevenDayResetAt(
			runtimePaths.windowState,
			runtimePaths.rateLimits,
		);
		const lastAlert = await readBurnAlertState(runtimePaths.burnAlert);
		if (
			!shouldSendAlertNow({ lastAlert, sevenDayResetAt, nowMs: Date.now() })
		) {
			return;
		}
		// Only advance the dedup clock when an alert was actually sent — otherwise
		// a missing notify config would silently suppress future alerts for 6h.
		if (runtimePaths.notify && hoursUntilWeeklyReset != null) {
			sendNotification(
				runtimePaths.notify,
				formatBurnPointAlert({
					weeklyPct: rateLimits.sevenDayPct,
					fiveHourPct: rateLimits.fiveHourPct,
					hoursUntilReset: hoursUntilWeeklyReset,
					heldTaskName: heldTask.name,
				}),
			);
			await writeBurnAlertState(runtimePaths.burnAlert, {
				sevenDayResetAt,
				atMs: Date.now(),
			});
		}
	} catch {
		// burn-point alerting is never fatal
	}
}
...
async function executeTask(
	...
	try {
		...
		const costMetrics = computeCostMetrics(windowBefore, windowAfter);
		await completeTask(queuePath, task.id, summary, costMetrics);
		...
		const errorMetrics = computeCostMetrics(windowBefore, windowAfter);
		if (rateLimited) {
			await requeueTask(
				queuePath,
				task.id,
			...
			await failTask(queuePath, task.id, errMsg.slice(0, 500), errorMetrics);
		}
...
export async function run(options: RunnerOptions): Promise<RunnerResult> {
	...
	// Independent of the scheduling decision: alert (don't fire) when past
	// burn point with a held reserve task queued.
	await maybeAlertBurnPoint(
		queuePath,
		rateLimits,
		hoursUntilWeeklyReset,
		runtimePaths,
...
async function drainQueue(
	...
	while (currentTask) {
		...
		if (outcome.success) {
			...
			// Exponential backoff with full jitter — instant retries after
			// failures just burn through the failure budget. Jitter prevents
			// synchronized retry storms when multiple cron jobs fail in lock-step.
...
// Explicit manual fire path for held (burn_point_only) tasks. Invoked ONLY by
// the fire-burn CLI, never the cron. The FIRST held task fires unconditionally:
// a manual fire-burn means the operator reviewed burn-status and approved
// spending now, so entry-time calendar/budget gating is intentionally skipped.
// Every SUBSEQUENT task in the same drain IS gated (calendar, reservation,
// budget/burn-point), and the drain stops fail-closed if the meter can't be read
// — so a multi-task burn never continues blind.
export async function runBurn(options: RunnerOptions): Promise<RunnerResult> {
	const { queuePath, dryRun = false, config } = options;
	const p = paths(config);
	const docketPath = config.docket?.reservationsPath ?? null;
	...
	const firstTask = await nextBurnPointTask(queuePath);
	if (!firstTask) {
		return {
			decision: "no_tasks",
			reason: "no held burn-point tasks queued",
			tasks: [],
			dryRun,
		};
	}
	...
	if (dryRun) {
		return {
			decision: "schedule",
			reason: "burn-point fire (dry run)",
			tasks: [
				{
					taskId: firstTask.id,
	...
	while (currentTask) {
		...
		if (outcome.success) {
			...
			await new Promise((r) =>
				setTimeout(r, computeBackoffMs(consecutiveFailures)),
			);
		}
		...
		if (busy) break;
		if (resv?.intensity === "heavy" && next.size !== "S") break;
		// Fail-closed: if the meter can't be read, don't continue the drain blind.
		if (!limits) break;
		if (limits.sevenDayPct >= 85) {
			const hrs = await getHoursUntilWeeklyReset(
				runtimePaths.windowState,
				runtimePaths.rateLimits,
			);
