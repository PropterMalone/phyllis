...
function makeTask(overrides: Partial<QueuedTask> = {}): QueuedTask {
	return {
		...
		project_dir: "/home/user/Projects/feed-hub",
		priority: 20,
...
SIZE: M
DIR: /home/user/Projects/feed-hub
DURATION: 611s
STATUS: done
...
describe("formatDigestHtml", () => {
	...
	it("shows unknown burn-point and unavailable reset when no live state", () => {
		const budget: WeeklyBudget = {
			resetTime: null,
			resetSource: "unknown",
			hoursUntilReset: 0,
...
describe("computeWeeklyBudget", () => {
	it("marks reset unknown and uses a rolling 7d window when no live data exists", () => {
		// No fresh rateLimits and no last-known reset → resetSource "unknown",
		// resetTime null (never a hardcoded guess), and the "since reset" window
		// rolls back 7d from now (2026-04-06 − 7d = 2026-03-30), so all 3 blocks count.
		const blocks = [
			makeBlock({
		...
		expect(budget.blocksSinceReset).toBe(3);
		expect(budget.tokensSinceReset).toBe(600000);
		expect(budget.resetSource).toBe("unknown");
		expect(budget.resetTime).toBeNull();
	...
	it("uses the last-known live reset (no freshness gate) instead of a hardcoded guess", () => {
		// Proxy idle past the staleness window → no fresh rateLimits, but the
		// last-captured reset instant is still a valid future timestamp.
		const now = new Date("2026-04-26T15:00:00Z"); // Sun
		const lastKnown = new Date("2026-04-28T03:00:00Z"); // Mon 11pm ET
		const budget = computeWeeklyBudget([], now, null, lastKnown);
		expect(budget.resetSource).toBe("last-known");
		expect(budget.resetTime).toBe("2026-04-28T03:00:00.000Z");
		expect(budget.hoursUntilReset).toBeCloseTo(36, 0);
		expect(budget.weeklyPct).toBeNull(); // still no weeklyPct without fresh header
	});

	it("uses rate-limits reset time, not hardcoded Friday", () => {
		// the user's actual case Sun afternoon: reset is Mon 23:00 ET = Tue 03:00 UTC.
