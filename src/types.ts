...
export interface QueuedTask {
	...
	// Per-task cost & token accounting (added 2026-05-24).
	// Source: ccusage active block delta captured before vs after task run.
	// Null/missing on legacy tasks and when ccusage data wasn't available.
	tokens_used?: number; // block.totalTokens delta
	cost_used?: number; // block.costUSD delta
	five_hour_pct_delta?: number; // window % consumed by this task
	seven_day_pct_delta?: number; // weekly % consumed by this task

	// Dead-letter tracking (added 2026-05-24).
	// Increments every time the task ends in `failed`. When it crosses the
	// dead-letter threshold (default 3), status auto-promotes to `dead` and
	// the task is excluded from nextTask().
	failure_count?: number;
	failure_history?: Array<{ at: string; reason: string }>;
}
