...
describe("buildThrottleEntry", () => {
	it("builds entry from active block", () => {
		const result = buildThrottleEntry([makeBlock()], "user", {
			fiveHourPct: 95,
			sevenDayPct: 79,
		});
	...
	it("falls back to most recent completed block", () => {
		...
		const result = buildThrottleEntry([completed], "user", null);
		expect(result).not.toBeNull();
	...
	it("prefers active over recent", () => {
		...
		const result = buildThrottleEntry([completed, active], "user", null);
	...
	it("skips gap blocks", () => {
		...
		const result = buildThrottleEntry([gap], "user", null);
		expect(result).toBeNull();
	});

	it("returns null for empty blocks", () => {
		expect(buildThrottleEntry([], "user", null)).toBeNull();
	});
	...
	it("includes custom notes", () => {
		const result = buildThrottleEntry(
			...
			"user",
			null,
			"Hit 95% warning in my-app session",
		);
		expect(result!.entry.notes).toBe("Hit 95% warning in my-app session");
	});

	it("omits rate_limits when null", () => {
		const result = buildThrottleEntry([makeBlock()], "user", null);
	...
	it("detects peak hour correctly", () => {
		...
		const result = buildThrottleEntry([peakBlock], "user", null);
		...
		const result2 = buildThrottleEntry([offPeakBlock], "user", null);
