...
export interface WeeklyBudget {
	resetTime: string | null; // ISO 8601 — next reset; null only when no live state has ever been written
	resetSource: "rate-limits" | "last-known" | "unknown"; // fresh live header / stale-but-real live value / no data
...
export function computeWeeklyBudget(
	...
	// Stale-but-real reset time (read WITHOUT the freshness gate) used for display
	// and window-anchoring when `rateLimits` is null. The reset is a fixed future
	// timestamp — it does not go stale the way weeklyPct does — so we still show the
	// true date even when the proxy has not captured traffic recently (e.g. overnight).
	displayResetAt: Date | null = null,
): WeeklyBudget {
	// Reset time: prefer Anthropic's fresh live header; else the last-known live
	// reset; never a hardcoded guess (Anthropic moves the reset point without
	// notice, so any hardcoded day/time is reliably wrong).
	let nextReset: Date | null;
	let resetSource: "rate-limits" | "last-known" | "unknown";
	if (rateLimits) {
		nextReset = rateLimits.weeklyResetAt;
		resetSource = "rate-limits";
	} else if (displayResetAt) {
		nextReset = displayResetAt;
		resetSource = "last-known";
	} else {
		nextReset = null;
		resetSource = "unknown";
	}
	// Window anchor for "since reset" aggregation: reset−7d when known, else a
	// rolling 7d so weekly totals stay meaningful without a known reset instant.
	const lastReset = nextReset
		? new Date(nextReset.getTime() - 7 * 24 * 60 * 60 * 1000)
		: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
	const hoursUntilReset = nextReset
	...
	return {
		resetTime: nextReset ? nextReset.toISOString() : null,
		resetSource,
		hoursUntilReset,
		hoursElapsed,
...
function formatBudgetHtml(budget: WeeklyBudget): string {
	const resetStr = budget.resetTime
		? new Date(budget.resetTime).toLocaleDateString("en-US", {
				weekday: "short",
				month: "short",
				day: "numeric",
				hour: "numeric",
...
<span style="color:#666;font-size:12px">${resetStr ? `Resets ${resetStr} ET` : "Reset time unavailable (no live state)"}</span>
</div>
...
/** Reset instant for display + window-anchoring — read WITHOUT the staleness
 * gate. Unlike weeklyPct (which goes stale), the reset is a fixed future
 * timestamp, so a value captured hours ago is still correct. This is what keeps
 * the overnight digest from falling back to a wrong hardcoded date when the proxy
 * has been idle past the freshness window. Returns null only when no live file
 * carries a sevenDayResetAt, or the value is already in the past (a missed reset
 * the proxy has not refreshed — better to show "unavailable" than a stale date). */
async function readResetTimeForDisplay(
	windowStatePath: string,
	rateLimitsPath: string,
): Promise<Date | null> {
	const ifFuture = (d: Date): Date | null =>
		d.getTime() > Date.now() ? d : null;
	try {
		const state = JSON.parse(await readFile(windowStatePath, "utf-8")) as {
			sevenDayResetAt?: number | string | null;
		};
		if (state.sevenDayResetAt != null) {
			const d =
				typeof state.sevenDayResetAt === "number"
					? new Date(state.sevenDayResetAt * 1000)
					: new Date(state.sevenDayResetAt);
	...
	try {
		const data = JSON.parse(await readFile(rateLimitsPath, "utf-8")) as {
			seven_day?: { resets_at?: number };
		};
		if (data.seven_day?.resets_at != null) {
			return ifFuture(new Date(data.seven_day.resets_at * 1000));
		}
	} catch {
		// no live reset available anywhere
	}
	return null;
}
...
export async function digest(options: DigestOptions): Promise<string> {
	...
	try {
		const [blocks, rateLimits, displayResetAt] = await Promise.all([
			fetchBlocks(),
			readRateLimits(windowStatePath, rateLimitsPath),
			readResetTimeForDisplay(windowStatePath, rateLimitsPath),
		]);
		budget = computeWeeklyBudget(blocks, now, rateLimits, displayResetAt);
	} catch {
